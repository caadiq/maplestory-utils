...
export const LOCATE = {
   ...
   *
   * **아이콘이 없으면 후보를 아예 안 내놓는 것**이 이 값의 목적이다.
   * 예전 0.50에서는 야누스가 없는 화면 40장 중 38장(95%)에서 평균 2.8개씩
   * 엉뚱한 후보가 떴다 — 마을에서 공유를 켜면 상관없는 칸이 60%·53%로 뜨는 게 그 탓이다.
   *
   * 실측으로 갈라지는 자리가 뚜렷하다:
   *   아이콘 없음 40장 — 최고 0.597
   *   아이콘 있음 708장 — 0.568짜리 한 장을 빼면 그 다음이 0.69 (그 사이가 통째로 비어 있다)
   * 그 빈 구간 가운데인 0.65로 두면 없는 화면에서 후보가 0개가 되고,
   * 잃는 건 708장 중 한 장(0.568)뿐이다. 그마저도 부재 최고점보다 낮아 어차피 못 믿을 값이다.
   */
  looseScore: 0.65,
