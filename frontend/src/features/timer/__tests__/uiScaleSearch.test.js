...
describe('LOCATE 임계값', () => {
  ...
  it('후보 하한이 부재 최고점 위에 있다 — 없으면 후보도 없어야 한다', () => {
    /*
     * 여기가 뒤집히면 야누스가 없는 화면에서 엉뚱한 칸이 후보로 뜬다.
     * 실측: 0.50이던 시절 부재 40장 중 38장(95%)에서 평균 2.8개씩 떴다.
     */
    expect(LOCATE.looseScore).toBeGreaterThan(ABSENT_MAX + 0.04)
  })

  it('후보 하한이 실제 정답 구간은 안 건드린다', () => {
    /*
     * 708장 중 0.568짜리 한 장을 빼면 그 다음 정답이 0.69다 — 그 사이가 통째로 비어 있어
     * 0.65면 잃는 게 그 한 장뿐이고, 그마저 부재 최고점보다 낮아 어차피 못 믿을 값이다.
     */
    expect(LOCATE.looseScore).toBeLessThan(0.69)
  })
