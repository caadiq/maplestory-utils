...
export const DETECT = {
   ...
   * 새벽/황혼 자동 전환.
   *
   * 지정된 자리를 내장 원본 둘과 대조해 어느 쪽인지 본다. 24×24로 줄여 봐도
   * 확실히 갈린다(실측): 새벽 대기 dawn 0.823 / dusk 0.363, 황혼 대기 dusk 0.885 /
   * dawn 0.411. 쿨타임 중에는 아이콘이 어두워지고 숫자가 얹혀 둘 다 0 근처로
   * 떨어지므로, 통과선을 두면 그때는 저절로 판단을 쉰다.
   */
  /*
   * 0.25초마다 본다. 1초로 뒀더니 실측 영상에서 **전환이 한 번도 안 일어났다** —
   * 황혼에서 새벽으로 바꾼 뒤 곧바로 설치해 버려서 '대기 상태'가 1초뿐이었고,
   * 1초 간격으로는 세 표를 모을 수가 없었다. 0.25초면 그 창에서도 모인다
   * (실측: 전환 2회를 0.3~0.9초 만에 정확히 잡고 오전환 0).
   * 24×24 벡터 두 번이라 네 배로 자주 봐도 비용은 사실상 없다.
   */
  modeCheckMs: 250,
  /*
   * 통과선 0.60은 **확장 UI를 통째로 막았다** — 아이콘이 작아 0.54~0.58까지만 나온다.
   * 0.50으로 내려도 안전하다: 실측 2,716장(새벽·확장·황혼)에서 틀린 표 0,
   * 아이콘이 없는 화면 40장에서는 최고 0.086이라 표가 아예 안 나온다.
   */
  modeScore: 0.50,
  /** 격차 0.25로 올리면 확장 UI가 통째로 막힌다(실측 격차 0.21) */
  modeMargin: 0.20,
  /** 이만큼 연달아 같은 답이 나와야 바꾼다 — 한 번의 오독으로 모드가 넘어가지 않게 */
  modeVotes: 3,
}
