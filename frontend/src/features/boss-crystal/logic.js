 ...
 * 캐릭터당 주간 결정석 수.
 *
 * 이건 **보스 처치 횟수** 한도이지 판매 한도가 아니다. 2026-09-17 패치에서
 * "캐릭터당 주간 결정 판매 12개 제한"이 삭제됐지만 처치 횟수는 그대로라,
 * 한 캐릭터가 한 주에 쥘 수 있는 결정석은 여전히 12개다 — 이 값은 안 바뀐다.
 * 공지 문구만 보고 지우면 계산이 부풀어 오른다.
 */
export const MAX_PER_CHARACTER = 12
/**
 * 월드당 주간 판매 한도. 지금은 게이지 표시에만 쓰고 수익에서 잘라내지는 않는다 —
 * 캐릭터가 8개(96개)를 넘어가면 실제로 팔 수 있는 것보다 많게 잡힌다.
 */
