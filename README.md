...
메이플스토리 플레이에 필요한 계산기 모음입니다. NEXON Open API로 캐릭터 정보를 불러와 보스 수익·심볼·헥사·해방·경험치를 계산하고, 화면 인식으로 동작하는 재획 타이머까지 제공하는 풀스택 서비스입니다.
...
- 📈 **경험치 계산기** — 지금 가진 컨텐츠·아이템으로 얻을 경험치를 레벨 기준으로 합산. 주간·잠수·아이템으로 분류하고, 최근 히스토리로 실측 페이스를 함께 보여줍니다
- 🔨 **장비 강화 기록** — 스타포스·큐브·잠재능력 이력을 아이템 단위로 묶어 사용 메소·성공률·등급업 확률 분석 (전체 기간, 날짜별 영구 캐시)
- ⏱️ **재획 타이머** — 게임 화면을 인식해 **솔 야누스 사이클 · 룬 등장 · VIP 부스터 종료 · 동작 반복 방지(동꼽)** 네 가지를 각각 알립니다. 새벽(설치기)·황혼(추가타) 모드 자동 전환, PiP 미니바
- 📰 **공지사항 · 썬데이 메이플** — 넥슨 공지/이벤트/업데이트/캐시샵 연동, 주차별 썬데이 메이플 자동 수집(node-cron)
- 🛠️ **관리자** — 보스·심볼·시즌·이미지·메뉴·타이머 알림음 관리 (S3 업로드, sharp 이미지 처리). 알림음은 여러 개를 한 번에 올리고 종류를 파일마다 지정
...
| **NEXON Open API** | 캐릭터·장비·심볼·헥사·스킬·강화 이력·공지 |
...
게임 창을 화면 공유로 받아 네 가지를 동시에 지켜봅니다. **브라우저 안에서만 처리하며 화면 영상은 서버로 보내지 않습니다.**

```
                  ┌─ 퀵슬롯 야누스 아이콘 ─▶ 쿨타임 숫자 ─▶ 사이클 판정 ┐
화면 공유 ────────├─ 상단 "등장" 문구 ─┐                                │
(getDisplayMedia) │  미니맵 분홍 마름모 ┴▶ 룬 알림 ─────────────────────├─▶ 알림 예약
                  ├─ 중앙 상단 "남은시간" ─▶ 남은 초 판독 ─▶ 0초 알림 ──┤   (AudioContext)
                  └─ 하단 경험치 숫자 ─▶ 멈춤 감지 ─▶ 동꼽 알림 ────────┘
```

야누스·룬·부스터는 **정규화 상호상관(NCC)** 으로 찾고, 무거운 계산은 워커에서 돌립니다.
...
- **룬 등장**은 **문구와 미니맵 표식 두 갈래**로 봅니다. 서로 약점이 정확히 달라서입니다 — 문구는 다른 UI 창에 가려지고(6차 전직 창 실측), 미니맵 표식은 룬이 발밑에 뜨면 내 마커에 덮입니다(실측 6회 중 1회). 한쪽이 막힐 때 다른 쪽은 멀쩡했습니다. 표식은 **룬을 먹을 때까지 남아** 자리를 비운 사이 문구가 지나가도 잡힙니다
- 문구 쪽은 '등장' 두 글자만 봅니다. 해방 후 문구·솔 에르다 안내도 같은 초록 계열이라 색·위치로는 갈라지지 않는데, 이 단어는 등장 문구에만 있습니다. 밝기가 아니라 **초록 우세 성분 `G − (R+B)/2`** 로 봐서 밝은 맵·어두운 맵 모두에서 잡힙니다 (일반룬·축복룬은 글꼴 크기가 달라 템플릿을 따로 둡니다)
...
- **동작 반복 방지(동꼽)** 는 화면 맨 아래 **경험치 숫자가 멈췄는지**로 봅니다. 걸리면 몹이 안 죽어 경험치가 완전히 멈추므로 신호가 깨끗합니다. 숫자를 읽지는 않고 그 줄의 **흰 글자 양을 세로로 합친 1차원 프로파일**을 프레임끼리 비교합니다 — 템플릿이 없어 오독이 없고, 자릿수가 늘어 글자가 밀려도 그대로 '변함'으로 잡힙니다
- **창 크기가 바뀌어도 지정한 자리를 지킵니다.** 창이 멈춰도 게임 화면은 한 박자 늦게 따라오므로(실측 2초), 화면이 잠잠해진 뒤에 재서 옮깁니다
- 알림음은 서버(RustFS + DB)에서 받아옵니다 — 관리자 화면에서 여러 개를 한 번에 올리고 정렬하며 코드 수정이 필요 없습니다
...
│   │                         #   enchant, exp, liberation(genesis-pass), notices,
│   │                         #   sunday-maple, images, menus, timer, admin
│   ├── services/             # character, hexa, enchant, exp, image,
