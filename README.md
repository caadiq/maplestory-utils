...
- **쿨타임은 보지 않습니다** — 지속시간(30렙 2분)이 쿨타임보다 길어서 야누스가 사라지기 전에 이미 쿨이 돌아와 있습니다. 정작 필요한 알림은 "사라지기 전에 다시 깔아라"입니다
- **깜빡임 대응** — 아이콘은 쿨타임 막바지 약 5초 동안 숫자가 사라지고 깜빡입니다. 밝아짐까지 상태 전환으로 받아들이면 매 사이클이 그만큼 어긋나므로, 밝아짐은 "다음 설치를 볼 준비가 됐다"는 내부 상태로만 쓰고 확정을 넉넉히(1.5초) 잡습니다
- **지속시간은 스킬 레벨에서 계산** — 10레벨 단위 계단식(1~9렙 60초 / 10~19렙 70초 / 20~29렙 80초 / 30렙 120초). 야누스는 버프 지속시간 증가의 영향을 받지 않아 레벨만으로 확정됩니다
- **알림 시점은 초 단위 직접 입력** — 사냥터마다 젠 주기가 달라 고정 프리셋을 쓸 수 없습니다. 야누스가 끊기지 않으려면 몬스터를 잡자마자 다시 깔아야 해서 보통 2젠 전쯤으로 잡습니다
- **알림은 `AudioContext` 예약** — `setTimeout`은 백그라운드 탭에서 1초 단위로 밀립니다
