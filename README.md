...
- ⏱️ **야누스 쿨타임 알림** - 화면 공유로 퀵슬롯 아이콘 밝기를 읽어 쿨타임을 실측하고 종료 직전에 알림 (PiP 미니바)
...
│       │   ├── janus/        # 야누스 쿨타임 알림 (화면 인식, 프론트 전용 — 백엔드 없음)
...
메이플 창을 화면 공유로 받아 **퀵슬롯 야누스 아이콘의 밝기만** 보고 쿨타임을 잽니다. 서버로 아무것도 보내지 않는 프론트 전용 기능입니다.

- **밝아짐/어두워짐 두 지점만 본다** — 어두워지면 설치, 다시 밝아지면 쿨타임 종료. **어두운 구간의 길이가 곧 실제 쿨타임**이라 사이클마다 자동으로 다시 측정되고, 날마다 타이밍이 어긋나는 문제가 생기지 않습니다
- **쿨타임 숫자 인식(OCR)은 쓰지 않습니다** — 위 방식으로 충분한지 실사용으로 확인한 뒤 필요하면 추가할 예정
- **알림은 `AudioContext` 예약** — `setTimeout`은 백그라운드 탭에서 1초 단위로 밀려서 "12초 전"이 15초 전이 됩니다
- **지속시간은 스킬 레벨에서 계산** (1렙 60초 / 10렙 70초 / 20렙 80초 / 30렙 120초, 사이는 보간). 야누스는 버프 지속시간 증가의 영향을 받지 않아 레벨만으로 확정됩니다
- **미니바를 Document PiP로 분리** 가능 — 게임이 창모드면 그 위에 뜹니다

> 브라우저 제약: 공유할 창을 **자동으로 고를 수 없고**(선택 UI는 브라우저가 그림), 공유 권한은 기억되지 않아 페이지를 새로 열면 다시 골라야 합니다. 게임 화면 위에 직접 오버레이를 그리는 것도 불가능합니다. 모바일은 화면 공유 자체가 없어 PC 전용입니다.
